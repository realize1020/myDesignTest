...
public abstract class AbstractBeanFactory extends SimpleAliasRegistry implements ConfigurableBeanFactory, AutowireCapableBeanFactory {
    ...
    protected void afterSingletonCreation(String beanName) {
        if (!this.singletonsCurrentlyInCreation.remove(beanName)) {
            throw new RuntimeException("Bean with name '" + beanName + "' is not in creation");
        }
    }
    ...
    protected <T> T doGetBean(String name, Class<T> requiredType) {
        ...
        if (beanDefinition.isSingleton()) {
            ...
            throw new RuntimeException("Unsupported scope '" + beanDefinition.getScope() + "' for bean '" + beanName + "'");
        }

        // 类型检查
        if (requiredType != null && !requiredType.isInstance(bean)) {
            throw new RuntimeException("Bean named '" + name + "' is expected to be of type '" + requiredType + "' but was actually of type '" + bean.getClass().getName() + "'");
        }
     ...
     * 获取bean的模板方法
     */
    protected abstract Object doGetBean(String beanName) throws RuntimeException;
